 ...
 *
 * [게임 방식]
 * - 초등 과학 교육과정 핵심 개념을 초등 눈높이로 각색한 4지선다 객관식 (10문제)
 * - 교사가 설정에서 영역(물질·생명·지구·에너지·상식)을 멀티셀렉트하면
 *   선택한 영역의 문제만 모아 셔플해 출제 (activity.scienceFields 배열)
 * - 보기 4개는 매판 셔플 → 정답 위치가 고정되지 않음
 * - 정답 1점, 점수 비율로 포인트 지급. 풀고 나면 한 줄 해설 표시
 */
import { useState, useRef, useEffect } from 'react'
import { SCIENCE_QUESTIONS, SCIENCE_FIELDS } from '../../data/scienceData'

const TOTAL = 10
const ALL_FIELD_KEYS = SCIENCE_FIELDS.map(f => f.key)
const FIELD_LABEL = Object.fromEntries(SCIENCE_FIELDS.map(f => [f.key, f.label]))

const shuffle = arr => [...arr].sort(() => Math.random() - 0.5)

function pickQuestions(fields) {
  const keys = Array.isArray(fields) && fields.length > 0 ? fields : ALL_FIELD_KEYS
  let pool = SCIENCE_QUESTIONS.filter(q => keys.includes(q.field))
  if (pool.length === 0) pool = SCIENCE_QUESTIONS  // 방어: 빈 풀이면 전체
...
export default function ScienceQuizGame({ activity, onComplete, onExit }) {
  const [questions] = useState(() => pickQuestions(activity?.scienceFields))
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-8">
      <div className="w-full max-w-md">
        ...
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-black text-carnival-navy">🔬 과학 상식 퀴즈</h1>
        ...
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-cyan-300 bg-cyan-50 text-cyan-700 text-sm font-bold">
            <span>🧪</span><span>{FIELD_LABEL[current.field] || '과학'}</span>
          </div>
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="w-full bg-gray-100 rounded-full h-3">
            <div className="h-3 rounded-full bg-cyan-400 transition-all duration-500"
        ...
        {done ? (
          ...
          <>
            ...
            <div className="card mb-5 py-6 px-5">
              <p className="text-xs text-carnival-navy/40 font-medium mb-2">{idx + 1}번 문제</p>
            ...
            <div className="grid grid-cols-1 gap-2.5">
              {current.options.map(opt => {
                ...
                if (picked === null) {
                  cls += ' hover:border-cyan-400 hover:bg-cyan-50'
                } else if (isAnswer) {
            ...
            {picked !== null && (
              <>
                ...
                {current.note && (
                  <p className="mt-2 text-sm text-carnival-navy/60 bg-cyan-50 border border-cyan-200 rounded-xl px-4 py-3 leading-relaxed">
                    💡 {current.note}
                  </p>
                )}
